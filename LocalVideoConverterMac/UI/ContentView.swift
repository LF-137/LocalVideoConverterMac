...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 10) {
                HStack {
                    Text("Batch Converter").font(.headline)
                    Spacer()
                    Button { showSettings = true } label: { Label("Settings", systemImage: "gearshape") }
                }
                
                Picker("Mode", selection: $savedMode) {
                    ForEach(ConversionMode.allCases) { mode in Text(mode.rawValue).tag(mode) }
                }
            ...
            if savedMode == .audioExtraction {
                HStack {
                    ...
                    Picker("", selection: $savedAudioFormat) {
                        ForEach(AudioExportFormat.allCases) { fmt in Text(fmt.displayName).tag(fmt) }
                    }
                    .frame(width: 100)
                    .onChange(of: savedAudioFormat) { _, newValue in converter.audioExportFormat = newValue }
                    Spacer()
                    Text("Expand items to select tracks").font(.caption).foregroundColor(.secondary)
                }
                .padding(.horizontal).padding(.bottom, 5).background(Color(NSColor.controlBackgroundColor))
            }
            ...
            ZStack {
                if converter.fileQueue.isEmpty {
                    VStack(spacing: 10) {
                        Image(systemName: "arrow.down.doc").font(.system(size: 40)).foregroundColor(.secondary)
                        Text("Drag & Drop videos here").font(.title3).foregroundColor(.secondary)
                    }
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                } else {
                    List {
                        ForEach(converter.fileQueue) { item in FileQueueRow(item: item, converter: converter) }
            ...
            // Footer
            VStack(spacing: 12) {
                if let error = converter.globalErrorMessage { Text(error).foregroundColor(.red).font(.caption) }
                if !converter.overallProgressMessage.isEmpty { Text(converter.overallProgressMessage).font(.caption).monospacedDigit() }
                ...
                HStack {
                    ...
                    if !converter.fileQueue.isEmpty {
                        Button("Clear All") { converter.clearQueue() }.disabled(converter.isBatchConverting)
                    ...
                    if converter.isBatchConverting {
                        ...
                        Button { converter.startBatch() } label: {
                            Text(savedMode == .videoConversion ? "Start Video Conversion" : "Extract Audio")
                        }
                        .buttonStyle(.borderedProminent).disabled(converter.fileQueue.isEmpty)
                    }
                }
            }
            .padding().background(Color(NSColor.controlBackgroundColor))
        ...
        .sheet(isPresented: $showSettings) {
            SettingsView(outputFormat: $savedOutputFormat, videoQuality: $savedVideoQuality, audioCodec: $savedAudioCodec, videoCodec: $savedVideoCodec)
                .onDisappear {
            ...
            HStack {
                if converter.mode == .audioExtraction {
                    Image(systemName: isExpanded ? "chevron.down" : "chevron.right")
                        .onTapGesture { withAnimation { isExpanded.toggle() } }.frame(width: 20)
                }
                VStack(alignment: .leading) {
                    Text(item.inputURL.lastPathComponent).font(.headline).truncationMode(.middle).lineLimit(1)
                    if let msg = item.successMessage { Text(msg).font(.caption).foregroundColor(.green) }
                    else if let err = item.errorMessage { Text(err).font(.caption).foregroundColor(.red) }
                    else { Text(item.status.displayName).font(.caption).foregroundColor(.secondary) }
                }
                Spacer()
                if item.status == .converting {
                    HStack(spacing: 8) { // Spacing for timer
                        ProgressView(value: item.progress).progressViewStyle(.linear).frame(width: 60)
                    ...
                    HStack {
                        ...
                        Button { converter.revealInFinder(item: item) } label: { Image(systemName: "magnifyingglass.circle") }
            ...
            if converter.mode == .audioExtraction && isExpanded {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    ForEach(item.audioTracks) { track in
                        HStack {
                            Toggle("", isOn: Binding(get: { track.isSelected }, set: { _ in converter.toggleTrackSelection(itemID: item.id, trackID: track.id) })).labelsHidden()
                            Text("Track \(track.index + 1)").font(.system(size: 11, weight: .bold)).frame(width: 50, alignment: .leading)
                            TextField("Filename", text: Binding(get: { track.customName }, set: { converter.updateTrackName(itemID: item.id, trackID: track.id, newName: $0) })).textFieldStyle(RoundedBorderTextFieldStyle()).font(.system(size: 11))
                        }
                    }
                    if item.audioTracks.filter({ $0.isSelected }).count > 1 {
                        Divider()
                        HStack {
                            Toggle("", isOn: Binding(get: { item.mergeSelectedTracks }, set: { _ in converter.toggleMerge(itemID: item.id) })).labelsHidden()
                            Text("Merge Selected").font(.system(size: 11, weight: .bold))
                            TextField("Merged Filename", text: Binding(get: { item.mergedTrackName }, set: { converter.updateMergedName(itemID: item.id, name: $0) })).textFieldStyle(RoundedBorderTextFieldStyle()).font(.system(size: 11))
                        }
                    }
                }
